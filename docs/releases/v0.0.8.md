...
## v0.0.8 Enhancements (2026-05-27) — Modular Intelligence Package

The knowledge system has been refactored into a modular `intelligence/` package with 11 single-responsibility modules. Backward compatible — all existing imports continue to work.

### New Architecture
```
context_engine/intelligence/
├── __init__.py       # Public API
├── schema.py         # DB schema + migration
├── storage.py        # Connection pool
├── evidence.py       # Confidence + scoring
├── decisions.py      # Architectural decisions
├── debugging.py      # Debug history
├── preferences.py    # Developer preferences
├── search.py         # FTS5 hybrid search
├── graph.py          # Graph-memory links
├── capture.py        # Auto-capture (36 patterns)
└── handlers.py       # Daemon/MCP handlers
```

### Evidence Model
All entries now track: confidence (0.0-1.0), frequency (occurrence count), source (manual/auto-detected), last_seen timestamp, and lifecycle status (active/deprecated/superseded).

### Confidence Escalation
- Manual source: confidence >= 0.8 minimum
- Auto-detected: confidence capped at 0.7, +0.1 per occurrence
- Repeated manual confirmation: up to 0.95

### Contradiction Detection
Value changes on existing entries are logged to `knowledge_conflicts` for human review. Old entries are marked `superseded`, never lost.

### Scoring Gate
Auto-capture scores candidates 0.0-1.0. Entries below 0.35 are discarded, eliminating noise from incidental keyword matches.

### 36 Preference Patterns
Auto-detection covers: package managers (3), databases (6), infra tools (7), languages (11), backends (2), infra styles (2), architecture patterns (2).

### Dynamic Retrieval
`dispatch_knowledge()` now accepts `task_context` for targeted `search_knowledge()` filtering — agents inject only relevant knowledge, not full dumps.
...
### 6. Codex MCP Integration Fix

Codex CLI v0.133.0+ manages MCP servers via a dedicated `codex mcp add` CLI subcommand
(stored in SQLite at `~/.codex/state_5.sqlite`), not through JSON config files.
The previous integration script only wrote `~/.codex/config.json` with developer instructions
but never registered the MCP server — making those instructions dead text.

**Fixed:** `scripts/codex-integration.py` now calls:
```bash
codex mcp add aihelper -- python3 context_engine/mcp_server.py
```

This registers the aihelper MCP server via stdio transport, giving Codex access to
all 20 aihelper tools (context, route, symbols, knowledge, graph, etc.).

**Verification:**
```bash
codex mcp list --json          # Confirm aihelper is listed
codex mcp get aihelper --json  # Show transport details
codex mcp remove aihelper       # Remove if needed
```
